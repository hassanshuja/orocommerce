...
const ShoppingListInlineEditingPlugin = InlineEditingPlugin.extend({
    ...
    enterEditMode(cell, event) {
        ...
        for (let i = 0; i < this.activeEditorComponents.length; i++) {
                if(this.activeEditorComponents[i].options.cell.model.attributes.quantity == this.activeEditorComponents[i].options.cell.$el.find('select[name="quantity"]').val() && this.activeEditorComponents[i].options.cell.model.attributes.unitCode == this.activeEditorComponents[i].options.cell.$el.find('select[name="unitCode"]').val()){               
                if(this.activeEditorComponents[i].cid != editorComponent.cid){                   
                     this.activeEditorComponents[i].trigger('cancelAction');
